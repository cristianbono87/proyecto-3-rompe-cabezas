...
var instrucciones = ['utilizar las flechas del teclado para mover las piezas', 'ordenar la imagen desplazando el espacio vacio', 'mientras mejos movimientos realices mas puntos obtendras'];
...
function agregarUltimoMovimiento(codigosDireccion){
  movimientos.push(codigosDireccion);

  // if (codigosDireccion === codigosDireccion.ABAJO) {
  //   movimientos.push(40);
  // }
...
function chequearSiGano() {
  ...
  for(var i = 0; i<grilla.length ; i++){
    for (var j = 0; j < grilla.length; j++) {
      var posicionGrilla = grilla[i][j];
      var posicionJuego = posicionGanadora[i][j];
      if(posicionGrilla === posicionJuego){
        x++
      }else{
        return;
      }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  var piezaAxB = grilla[filaPos1][columnaPos1];
  var piezaBxA = grilla[filaPos2][columnaPos2];

  console.log(piezaAxB, piezaBxA);
  grilla[filaPos1][columnaPos1] = piezaBxA;
  grilla[filaPos2][columnaPos2] = piezaAxB;
}
